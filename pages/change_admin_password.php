...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($admin_username) && !empty($current_password) && !empty($new_password) && !empty($confirm_password)) {
        if ($new_password === $confirm_password) {
            ...
            // 检查当前密码是否正确
            $check_sql = "SELECT * FROM admin WHERE Admin_name = ?";
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                if (password_verify($current_password, $admin['HASH_Value'])) {
                    // 更新密码
                    $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
                    $update_sql = "UPDATE admin SET HASH_Value = ? WHERE Admin_name = ?";
                ...
                $_SESSION['password_error'] = "管理员账号不存在!";
            }
            mysqli_close($link);
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Change Admin Password - CrossBorder Insurance</title>
...
<body>
    ...
    <div class="change-password-container">
        <div class="change-password-box">
            ...
            <p class="welcome-text">Please enter your admin username and new password</p>

            <!-- 修改密码表单 -->
            <form action="change_admin_password.php" method="POST" id="changePasswordForm" onsubmit="return validateForm()">
                <div class="form-group">
                    <input type="text" name="admin_username" class="form-control" placeholder="Enter admin username" id="admin-username">
    ...
    <script>
        function validateForm() {
            ...
            if (!username.value.trim()) {
                ...
                document.getElementById('admin-username-error').textContent = 'Please enter admin username';
                isValid = false;
            }
